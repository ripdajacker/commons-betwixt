...
public class XMLIntrospectorHelper {
    ...
    public static NodeDescriptor createDescriptor(
        ...
        if (log.isTraceEnabled()) {
            log.trace(
                    "Creating descriptor for property: name="
                            + name + " type=" + type);
        }
        ...
        if (isPrimitiveType(type)) {
            ...
            nodeDescriptor.setTextExpression(new MethodExpression(readMethod, name));

            if (writeMethod != null) {
                nodeDescriptor.setUpdater(new MethodUpdater(writeMethod, name));
            }
            ...
            loopDescriptor.setContextExpression(
                    new IteratorExpression(new MethodExpression(readMethod, name))
            );
            ...
            ElementDescriptor elementDescriptor = new ElementDescriptor();
            elementDescriptor.setContextExpression(new MethodExpression(readMethod, name));
            if (writeMethod != null) {
                elementDescriptor.setUpdater(new MethodUpdater(writeMethod, name));
            }
    ...
    public static void configureProperty(
        ...
        if (isPrimitiveType(type)) {
            elementDescriptor.setTextExpression(new MethodExpression(readMethod, propertyDescriptor.getName()));
            ...
            elementDescriptor.setContextExpression(
                    new IteratorExpression(new MethodExpression(readMethod, propertyDescriptor.getName()))
            );

            writeMethod = null;
        } else {
            log.trace("Standard property");
            elementDescriptor.setContextExpression(new MethodExpression(readMethod, propertyDescriptor.getName()));
        }
        ...
        if (updateMethodName == null) {
            ...
            if (writeMethod != null) {
                elementDescriptor.setUpdater(new MethodUpdater(writeMethod, propertyDescriptor.getName()));
            }
            ...
            if (updateMethod == null) {
                ...
                elementDescriptor.setUpdater(new MethodUpdater(updateMethod, propertyDescriptor.getName()));
    ...
    public static void configureProperty(
        ...
        if (isLoopType(type)) {
            log.warn(
                    "Using loop type for an attribute. Type = "
                        ...
                        if ((types.length == 1) || types.length == 2) {
                            ...
                            if (descriptor != null) {
                                ...
                                if (!isMapDescriptor && types.length == 1) {
                                    ...
                                    descriptor.setUpdater(new MethodUpdater(method, propertyName));
