...
public class TestRecursion extends AbstractTestCase {
    ...
    public void testReadwithCollectionsInElementRoundTrip() throws Exception {
        ...
        assertEquals(
                "Element one one name", "element11", elementOneOne
                        .getName());
        Element elementOneTwo = (Element) elements.get(1);
        assertEquals(
                "Element one two name", "element12", elementOneTwo
                        .getName());
        assertEquals(
                "Element one two children", 0, elementOneTwo.getElements()
                        .size());
        ...
        assertEquals(
                "Element one one one name", "element111", elementOneOneOne
                        .getName());
        Element elementOneOneTwo = (Element) elements.get(1);
        assertEquals(
                "Element one one two name", "element112", elementOneOneTwo
                        .getName());
    ...
    public void testReadWithoutCollectionsInElementRoundTrip() throws Exception {
        ...
        elements = elementOne.getElements();
        assertEquals(
                "Number of child elements in first element", 2, elements
                        .size());
        Element elementOneOne = (Element) elements.get(0);
        ...
        elements = elementOneOne.getElements();
        assertEquals(
                "Number of child elements in element 11", 2, elements
                        .size());
        Element elementOneOneOne = (Element) elements.get(0);
        assertEquals(
                "111 element name", "element111", elementOneOneOne
                        .getName());

        assertEquals(
                "111 child elements ", 0, elementOneOneOne.getElements()
                        .size());

        Element elementOneOneTwo = (Element) elements.get(1);
        assertEquals(
                "112 element name", "element112", elementOneOneTwo
                        .getName());
        assertEquals(
                "112 child elements ", 0, elementOneOneTwo.getElements()
                        .size());

        elements = elementOneTwo.getElements();
        assertEquals(
                "Number of child elements in element 12", 0, elements
                        .size());

        elements = elementTwo.getElements();
        assertEquals(
                "Number of child elements in element 2", 0, elements
                        .size());
    ...
    public void testCycleReferences() throws Exception {
        ...
        assertSame(
                "Cycle did not result in the same reference", result, result
