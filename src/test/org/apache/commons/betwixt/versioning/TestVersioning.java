...
public class TestVersioning extends AbstractTestCase {
    ...
    public void testIntrospection() throws Exception {
        ...
        java.util.List<ElementDescriptor> elementDescriptors = beanInfo
                .getElementDescriptor().getElementDescriptors();
        assertEquals("Need 2 element descriptors", 2, elementDescriptors.size());
        ...
        assertEquals(
                "1", element1Descriptor.getOptions().getValue(
                        "version-from"));
        ...
        assertEquals(
                "2", element2Descriptor.getOptions().getValue(
                        "version-from"));
        assertNull(element2Descriptor.getOptions().getValue("version-until"));

        // 2 Attribute descriptors
        List<AttributeDescriptor> attributeDescriptors = beanInfo.getElementDescriptor().getAttributeDescriptors();
        assertEquals(
                "Need 2 attribute descriptors", 2,
        ...
        assertEquals(
                "2", attribute1Descriptor.getOptions().getValue(
                        "version-from"));
        ...
        assertEquals(
                "1", attribute2Descriptor.getOptions().getValue(
                        "version-from"));
        assertEquals(
                "2", attribute2Descriptor.getOptions().getValue(
                        "version-until"));
    ...
    public void testWrite1() throws Exception {
        ...
        xmlAssertIsomorphicContent(
                parseString(expected), parseString(written),
                true);
